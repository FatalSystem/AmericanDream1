...
const PaymentComponent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (success === "true") {
      ...
      // Otherwise if we have product, quantity and amount
        ...
        console.log("🔄 Recording payment from URL parameters:", {
          product,
          quantity,
          amount,
        });
      ...
      // If we have neither session ID nor parameters
        ...
        console.warn(
          "⚠️ Payment success but missing session_id and product parameters"
        );
        toast.warning(
          "Payment successful but we couldn't get all details. Please contact support.",
          {
            theme: "dark",
          }
        );
  ...
  const getStripeSessionDetails = async (sessionId: string) => {
    try {
      ...
      const stripeSessionResponse = await api.get(
        `/stripe/check-session/${sessionId}`
      );
      ...
      console.log("⭐ Sending payment to API:", {
        ...
        requestData: {
          ...
          class_type_name:
            classTypeId === 1 ? "Trial-Lesson" : "Regular-Lesson",
      ...
      console.log(
        `✅ Stripe payment recorded: ${quantity} lessons for $${amountInDollars}`
      );

      // Show success notification with more details
      toast.success(
        `Payment successful! ${quantity} ${
          classTypeId === 1 ? "trial" : "regular"
        } lesson(s) added.`,
        {
          theme: "dark",
      ...
      if (res.data.payments) {
        ...
        console.log(
          `✅ Updated payments table with ${res.data.payments.length} records`
        );
      }
      ...
      toast.error(
        "Payment was successful, but we couldn't update your lessons. Please contact support.",
        {
          theme: "dark",
  ...
  const recordPaymentFromParams = async (
    ...
    try {
      ...
      if (isNaN(quantity) || isNaN(amount) || quantity <= 0 || amount <= 0) {
        throw new Error(
          `Invalid amount or quantity: amount=${amount}, quantity=${quantity}`
        );
      }
      ...
      console.log(`✅ Payment API Response:`, res.data);
      console.log(
        `✅ Stripe payment recorded: ${quantity} lessons for $${amount}`
      );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const formattedPayments = (paymentsRes.data.payments || []).map(
        (payment: Payment) => {
  ...
  const openEditPayment = (payment: Payment) => {
    ...
    if (payment.payment_time) {
      ...
      setPaymentTime(
        userTimeString ? dayjs(`2000-01-01 ${userTimeString}`) : null
      );
    } else {
      setPaymentTime(null);
    }
  ...
  const updatePayment = async () => {
    ...
    try {
      ...
      // Check if we need to adjust the date due to timezone differences
      let paymentDateStr = paymentDate
  ...
  const downloadCSV = () => {
    ...
    const csvData = payments.map((payment) => {
      ...
      const userTimeString = payment.payment_time
        ? convertTimeToUserTimezone(payment.payment_time, timezone)
        : null;
      ...
      if (payment.payment_time) {
        ...
        if (dayShift !== 0) {
          // Adjust the date by the number of days shifted
          adjustedDate = dayjs(payment.payment_date)
            .add(dayShift, "day")
            .format("YYYY-MM-DD");
        }
      }
      ...
      return {
        ...
        "Payment Time": userTimeString
          ? formatTime(userTimeString, "HH:mm")
          : "-",
      ...
      },
      {
        ...
        render: (_: any, record: any) => {
          ...
          if (record.payment_time) {
            ...
            if (dayShift !== 0) {
              // Adjust the date by the number of days shifted
              adjustedDate = dayjs(record.payment_date)
                .add(dayShift, "day")
                .format("YYYY-MM-DD");
            }
          }
              ...
              {stripeScriptError ? (
                <div className="flex size-full flex-col items-center justify-center bg-[#0f172a] p-8">
                  ...
                  <p className="mb-4 text-gray-400">
                    We couldn't load the payment system. Please try again later.
                  </p>
                  <Button
                    gradientDuoTone="purpleToBlue"
                    onClick={() => {
