      ...
      const processedTeacherData = teachers.map(teacher => {
        ...
        if (isNazar) {
          ...
          lessonsInTargetRange.forEach(lesson => {
            ...
            console.log(`=== NAZAR LESSON DETAIL: ID=${lesson.id}, Date=${lessonDate}, Type=${lesson.class_type?.name}, Status=${lesson.class_status} ===`);
          });
          ...
          Object.entries(dateCounter).forEach(([date, count]) => {
            if (count > 1) {
              ...
              lessonsOnThisDate.forEach(lesson => {
                console.log(`=== DUPLICATE DETAIL: ID=${lesson.id}, Type=${lesson.class_type?.name}, Status=${lesson.class_status} ===`);
              });
            }
          });
        }
        ...
        const nazarDebugLessons: {date: string, type: string, status: string, id: string}[] = [];
        ...
        teacherLessons.forEach((lesson: any) => {
          ...
          if (isNazar) {
            console.log(`=== NAZAR DEBUG: Processing lesson: ${lessonDate} - ${classType} - ${classStatus} ===`);
          ...
          // Only log for Nazar
          if (isNazar) {
            console.log(`=== NAZAR DEBUG: Added lesson to ${key}: date=${lessonDate}, count=${lessonCounters[key].count}, rate=${rate} ===`);
          }
        });
        ...
        <div className="flex w-full flex-col items-start gap-3 xs:flex-row xs:items-center sm:w-auto">
          ...
          {loading && (
            <div className="ml-2 flex animate-pulse items-center text-blue-500">
              <svg className="mr-2 size-5" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <Table
                ...
                summary={(pageData) => {
                  ...
                  return (
                    <Table.Summary fixed>
                      <Table.Summary.Row>
                        <Table.Summary.Cell index={0} colSpan={2} className="bg-gray-100 font-bold dark:bg-gray-800">
                          Total
                        </Table.Summary.Cell>
